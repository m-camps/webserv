 ...
 * @Listen will "say" to the socket to incoming connections.
 *
...
//		std::exit(EXIT_FAILURE);
	...
	while (true) {
		...
		int32_t new_socket = accept(socket_fd, reinterpret_cast<sockaddr *>(&address),
		...
		if (write(new_socket , &hello , hello.size() + 1) < 0)
			std::exit(EXIT_FAILURE);
